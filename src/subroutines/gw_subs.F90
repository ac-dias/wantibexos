...
subroutine ovpsqr(dft,w90basis,wf1,sk1,wf2,sk2,ovpmn)
	...
	character(len=1) :: dft
	integer :: w90basis
	complex,dimension(w90basis,w90basis) :: sk1,sk2,skaux
...
!subroutines for g0w0_algo normal

subroutine polarization_normal(dft,ink,fermishift,sysdim,ngrid,nk,rlat,omega,w90basis,systype,eta,ek,ekq,wfq,kovp,pol) !given polarization for a given q for all w-freq
	...
	real :: eta
	real,dimension(nk,w90basis) :: ek,ekq
	complex,dimension(nk,w90basis,w90basis) :: wfq
	complex,dimension(nk,w90basis,w90basis) :: kovp
	  ...
	  do i=1,nk
	    do j=1,w90basis
	      do k=1,w90basis
	    	...
	    	call ovpsqr(dft,w90basis,wfq(ink,j,:),kovp(ink,:,:),wfq(i,k,:),kovp(i,:,:),ovpmnkkq)
	    	
	    	pol = pol + ((aux1/aux2)*ovpmnkkq)
...
subroutine self_en_x_normal(dft,nkp,w90basis,sysdim,rlat,n,kpt,ngrid,nq,qpt,nocpq,coultype,ediel,lc,ez,w,r0,tolr,wfq,kovp,sxnk)
	...
	complex,dimension(nq,w90basis,w90basis) :: wfq
	complex,dimension(nq,w90basis,w90basis) :: kovp
	...
	case default
	  ...
	  do j=1,nocpq(i)
	  	...
	  	call ovpsqr(dft,w90basis,wfq(nkp,n,:),kovp(nkp,:,:),wfq(i,j,:),kovp(i,:,:),ovpmnkq)
	  		  	
	  	
	  	sxnk = sxnk - (waux*vq*ovpmnkq)
...
end subroutine self_en_x_normal

subroutine self_c_znk_normal(dft,nkp,w90basis,sysdim,rlat,ngrid,nq,fermishift,n,enk,emkmq,kpt,omega,nomega,w0,wfq,kovp,eta,scnk,znk)
	...
	do i=1,nq
	  do j=1,w90basis
	    ...
	    do k=1,nomega-1
	      ...
	      call ovpsqr(dft,w90basis,wfq(nkp,n,:),kovp(nkp,:,:),wfq(i,j,:),kovp(i,:,:),ovpmnkkmq)	    
	      
	      scnk = scnk + (integral1*ovpmnkkmq*waux)
	      
	      dscnk = dscnk + (integral2*ovpmnkkmq*waux)
...
end subroutine self_c_znk_normal
