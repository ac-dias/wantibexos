...
function v3davg(kpt1,kpt2,ed,rlat,ngrid,tolr)
	...
	real,dimension(3) :: kpt1,kpt2
	real,dimension(3,3) :: rlat
	real :: ed,tolr,modk,vc,vbz,v3davg
	...
	if (modk .lt. tolr) then
		! BSE matrix construction is OpenMP parallel, so calculate the
		! mesh-dependent quadrature once and share it among all threads.
		!$omp critical (v3davg_cache)
		if (.not. cache_valid) then
			cached_value=real(v3davg_cell(ngrid,rlat))
			cached_ngrid=ngrid
			cached_rlat=rlat
			cache_valid=.true.
...
! Average 1/q**2 over the parallelepiped generated by b1/ngrid(1),
! b2/ngrid(2), and b3/ngrid(3).  The cell is decomposed into twelve
! Duffy-mapped tetrahedra, making the q=0 singularity analytic and leaving a
! smooth two-dimensional quadrature on the faces.
real(kind=8) function v3davg_cell(ngrid,rlat)
	...
	real(kind=8),dimension(3) :: a1,a2,a3,cross23,cross31,cross12
	...
	real(kind=8) :: volume,cell_volume,integral,jacobian
	real(kind=8) :: t,u,wt,wu,avg_kernel,grid_size
	integer,dimension(4,6) :: face
	integer :: iface,itri,i,j
	...
	cross23(1)=dq2(2)*dq3(3)-dq2(3)*dq3(2)
	cross23(2)=dq2(3)*dq3(1)-dq2(1)*dq3(3)
	cross23(3)=dq2(1)*dq3(2)-dq2(2)*dq3(1)
	cell_volume=abs(dq1(1)*cross23(1)+dq1(2)*cross23(2)+dq1(3)*cross23(3))
		...
		do itri=1,2
			...
			do i=1,nq
				...
				do j=1,nq
					u=0.5d0*(xg(j)+1.0d0)
					wu=0.5d0*wg(j)
					direction=(1.0d0-t)*va+t*((1.0d0-u)*vb+u*vc)
					integral=integral+wt*wu*jacobian*t/sum(direction*direction)
				end do
			end do
		end do
	end do
