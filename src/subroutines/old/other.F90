subroutine lvl(w90basis,vec,lz)
	...
	complex,parameter :: imag = cmplx(0d0,1d0)

	real, dimension(3) :: mag !direção do vetor magnetização do substrato

	complex,dimension(w90basis) :: vec
	complex,dimension(w90basis) :: vcconj

	complex,dimension(w90basis) :: zvvaux

	complex,dimension(w90basis,w90basis) :: splz

	real,parameter :: ct1= 1.0
	real,parameter :: exc= 1.0

	complex :: al,aj,ak,am !termos auxiliares

	complex,dimension(5,5) :: dcor !correção orbitais d
	complex,dimension(3,3) :: pcor !correção orbitais p
	complex,dimension(1,1) :: scor !correção obitais s
...
subroutine exch(w90basis,exc,excl,mag,hexc)
	...
	complex,parameter :: imag = cmplx(0d0,1d0)
	real,parameter :: ct1 = 0.5d0
	real,parameter :: ct2 = sqrt(3d0)/(2d0)
	...
	complex,dimension(3,3) :: xexcuu,xexcdd,xexcud,xexcdu 
	complex,dimension(5,5) :: mexcuu,mexcdd,mexcud,mexcdu

	complex,dimension(w90basis/2,w90basis/2) :: hexcuu,hexcud,hexcdu,hexcdd

	complex :: axy,al,aj,ak,am
