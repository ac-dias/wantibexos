...
function v3davggw(kpt1,kpt2,ngrid,rlat,tolr)
    ...
    real,dimension(3,3) :: rlat
    integer,dimension(3) :: ngrid
    real :: tolr,modk,v3davggw
    real :: vc
    real(kind=8) :: vbz8,factor8,value8
    real(kind=8) :: v3davg_cellgw
    ...
    if (modk .le. tolr) then
        ...
        if (.not. cache_valid) then
            cached_value=v3davg_cellgw(ngrid,rlat)
            cached_ngrid=ngrid
            cached_rlat=rlat
            cache_valid=.true.
    ...
    v3davggw=real(value8)

end function v3davggw

real(kind=8) function v3davg_cellgw(ngrid,rlat)
    ...
    v3davg_cellgw=cic*avg_kernel!/(grid_size*abs(volume))

end function v3davg_cellgw
...
function v2dtavggw(kpt1,kpt2,ngrid,rlat,tolr)
	...
	real,dimension(3,3) :: rlat
	integer,dimension(3) :: ngrid
	real :: tolr,modk,v2dtavggw
	real :: vc,vbz,factor,gpar,gz,rc
	real :: aux1,aux2,aux3,aux4,aux5
	real(kind=8) :: v2dtavg_cellgw
	...
	if ((gpar .lt. tolr) .and. (gz .lt. tolr)) then
		...
		if (.not. cache_valid) then
			cached_value=real(v2dtavg_cellgw(ngrid,rlat))
			cached_ngrid=ngrid
			cached_rlat=rlat
			cache_valid=.true.
		else if (any(cached_ngrid .ne. ngrid) .or. any(cached_rlat .ne. rlat)) then
		...
		v2dtavggw=cached_value
		!$omp end critical (v2dtavg_cache)

	else if ((gpar .lt. tolr) .and. (gz .ge. tolr)) then
		v2dtavggw=(vbz*cic)*(factor/(modk*modk)) &
...
real(kind=8) function v2dtavg_cellgw(ngrid,rlat)
	...
	grid_size=dble(ngrid(1))*dble(ngrid(2))*dble(ngrid(3))
	v2dtavg_cellgw=cic*avg_kernel!/(grid_size*abs(volume))

end function v2dtavg_cellgw
