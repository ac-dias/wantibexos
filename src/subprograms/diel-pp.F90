...
subroutine spoptprop(numbse,outputfolder,tmax,ni,ns)
	...
	OPEN(UNIT=100, FILE=trim(outputfolder)//"ipa_diel_xx.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xx input file"
	OPEN(UNIT=101, FILE=trim(outputfolder)//"ipa_diel_xy.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xy input file"
	OPEN(UNIT=102, FILE=trim(outputfolder)//"ipa_diel_xz.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xz input file"
	OPEN(UNIT=103, FILE=trim(outputfolder)//"ipa_diel_yy.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_yy input file"
	OPEN(UNIT=104, FILE=trim(outputfolder)//"ipa_diel_yz.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_yz input file"
	OPEN(UNIT=105, FILE=trim(outputfolder)//"ipa_diel_zz.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_zz input file"
	...
	OPEN(UNIT=300, FILE=trim(outputfolder)//"ipa_extinction_coef.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_extinction_coef output file"
	OPEN(UNIT=400, FILE=trim(outputfolder)//"ipa_reflectibility.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_reflectibility output file"
	...
	OPEN(UNIT=900, FILE=trim(outputfolder)//"ipa_transmittance.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_transmittance.dat output file"
	OPEN(UNIT=901, FILE=trim(outputfolder)//"ipa_reflectance.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_reflectance output file"
	OPEN(UNIT=902, FILE=trim(outputfolder)//"ipa_absorptance.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_absorptance output file"						
