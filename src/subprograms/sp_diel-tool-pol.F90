...
subroutine spdielrawpol(nthreads,dft,outputfolder,renorm,params,ngrid,nc,nv,ebse0,ebsef,numbse,sme)
	...
	real,allocatable,dimension(:) :: exxf,eyyf,ezzf 
	real,allocatable,dimension(:) :: espf,esmf 
	...
	real,allocatable,dimension(:,:) :: dielfsp,dielfsm	
	...
	OPEN(UNIT=300, FILE=trim(outputfolder)//"ipa_diel-pol_x.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_x output file"
	OPEN(UNIT=301, FILE=trim(outputfolder)//"ipa_diel-pol_y.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_y output file"
	OPEN(UNIT=302, FILE=trim(outputfolder)//"ipa_diel-pol_z.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_z output file"
	OPEN(UNIT=303, FILE=trim(outputfolder)//"ipa_diel-pol_sp.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_sp output file"
	OPEN(UNIT=304, FILE=trim(outputfolder)//"ipa_diel-pol_sm.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_sm output file"
