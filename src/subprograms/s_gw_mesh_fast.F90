subroutine sgw_mesh_fast(nthreads,outputfolder,ngrid,ifactor,smegw,ktolgw,params,edielgw,&
	...
	real,allocatable,dimension(:,:,:,:) :: mmnkq	
	...
	do i=1,nkpt
	 do j=1,nomega
		...
		call polarization(fermishift,sysdim,ngrid,nkpt,rlat,omega(j),w90basis,systype,&
		                   smegw,keigv,qpqeigv(i,:,:),mmnkq(:,:,i,:),pol(i,j))
