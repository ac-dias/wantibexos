...
subroutine bsebnds(nthreads,outputfolder,calcparms,ngrid,nc,nv,numdos, &
	...
	!$omp parallel do default(shared) private(i2,j,l,h,eaux,vaux)
		...
		call OPENBLAS_SET_NUM_THREADS(1)
			...
			call eigsys(nthreads,dft,systype,scs,exc,nocpq(i2),ffactor,qpt(i2,1),qpt(i2,2),qpt(i2,3),w90basis,nvec,&
			             rlat,rvec,hopmatrices,ihopmatrices,ovp,efermi,eaux,vaux,nocpf,fermishift,mag)
#ifdef MKL
		call MKL_SET_NUM_THREADS(nthreads)
#endif		
