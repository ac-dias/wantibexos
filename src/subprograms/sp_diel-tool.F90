...
subroutine spdielraw(nthreads,dft,outputfolder,renorm,params,ngrid,nc,nv,ebse0,ebsef,numbse,sme)
	...
	real,allocatable,dimension(:) :: exxf,exyf,exzf 
	real,allocatable,dimension(:) :: eyyf,eyzf,ezzf 
	...
	OPEN(UNIT=300, FILE=trim(outputfolder)//"ipa_diel_xx.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xx output file"
	OPEN(UNIT=301, FILE=trim(outputfolder)//"ipa_diel_xy.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xy output file"
	OPEN(UNIT=302, FILE=trim(outputfolder)//"ipa_diel_xz.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_xz output file"
	OPEN(UNIT=303, FILE=trim(outputfolder)//"ipa_diel_yy.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel_yy output file"
	OPEN(UNIT=304, FILE=trim(outputfolder)//"ipa_diel_yz.dat",STATUS='unknown', IOSTAT=erro)
