...
subroutine spoptproppol(numbse,outputfolder)
	...
	OPEN(UNIT=100, FILE=trim(outputfolder)//"ipa_diel-pol_x.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_x input file"
	OPEN(UNIT=101, FILE=trim(outputfolder)//"ipa_diel-pol_y.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_y input file"
	OPEN(UNIT=102, FILE=trim(outputfolder)//"ipa_diel-pol_z.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_z input file"
	OPEN(UNIT=103, FILE=trim(outputfolder)//"ipa_diel-pol_sp.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_sp input file"
	OPEN(UNIT=104, FILE=trim(outputfolder)//"ipa_diel-pol_sm.dat",STATUS='old', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_diel-pol_sm input file"
	...
	OPEN(UNIT=300, FILE=trim(outputfolder)//"ipa_extinction_coef-pol.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_extinction_coef-pol output file"
	OPEN(UNIT=400, FILE=trim(outputfolder)//"ipa_reflectibility-pol.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_reflectibility-pol output file"
	OPEN(UNIT=500, FILE=trim(outputfolder)//"ipa_absorption_coef-pol.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_absorption_coef-pol output file"
	OPEN(UNIT=600, FILE=trim(outputfolder)//"ipa_en_loss_func-pol.dat",STATUS='unknown', IOSTAT=erro)
	if (erro/=0) stop "Error opening ipa_en_loss_func-pol output file"		
	OPEN(UNIT=700, FILE=trim(outputfolder)//"ipa_opt_cond_real-pol.dat",STATUS='unknown', IOSTAT=erro)
