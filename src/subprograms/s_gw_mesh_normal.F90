subroutine sgw_mesh_normal(nthreads,outputfolder,ngrid,ifactor,smegw,ktolgw,params,edielgw,&
	...
	real,allocatable,dimension(:,:) :: keigv,ebands
	complex,allocatable,dimension(:,:,:) :: kvector
	integer,allocatable,dimension(:) :: nocpq,nocpk
	complex,allocatable,dimension(:,:,:) :: kovp
	...
	do i=1,nkpt
	 do j=1,w90basis
	  ...
	  call self_en_x_normal(dft,i,w90basis,sysdim,rlat,j,kpt(i,:),ngrid,nkpt,kpt,nocpk,coultypegw,edielgw,&
	               lcgw,ezgw,wgw,r0gw,ktolgw,kvector,kovp,selfx(j,i))
...
if (selfxonly) then
	...
	do i=1,nkpt
	 do j=1,nomega
		...
		call polarization_normal(dft,i,fermishift,sysdim,ngrid,nkpt,rlat,omega(j),w90basis,systype,&
		                   smegw,keigv,qpqeigv(i,:,:),kvector,kovp,pol(i,j))
	...
	do i=1,nkpt
	 do j=1,w90basis
	  ...
	  call self_c_znk_normal(dft,i,w90basis,sysdim,rlat,ngrid,nkpt,fermishift,j,&
	                  keigv(i,j),kmqeigv(i,:,:),kpt(i,:),omega,nomega,w0,kvector,kovp,smegw,selfc(j,i),znk(j,i))
...
end subroutine sgw_mesh_normal	
