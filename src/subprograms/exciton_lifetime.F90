...
subroutine lifetime(sysdim,numbse,ngrid,rlat,nc,nv,outputfolder)
	...
	integer :: erro
	real,allocatable,dimension(:,:) :: refr
	real,allocatable,dimension(:,:) :: fosc
	integer :: dimbse,i,j
	
	real :: resx,resy,resz
	real :: refraux,foscaux
	real,dimension(4) :: lft
	
	
	!arquivo de entrada

	OPEN(UNIT=100, FILE=trim(outputfolder)//"bse_oscf.dat",STATUS='old', IOSTAT=erro)
...
subroutine lifetimepol(sysdim,numbse,ngrid,rlat,nc,nv,outputfolder)
	...
	real :: resx,resy,resz
	real :: refraux,foscaux
	real,dimension(6) :: lft
	
	
	!arquivo de entrada

	OPEN(UNIT=100, FILE=trim(outputfolder)//"bse_oscf-pol.dat",STATUS='old', IOSTAT=erro)
